   ...
   <div th:fragment = "header">
      <header class="d-flex flex-wrap justify-content-center py-3 mb-1 border-bottom">
         <a href="shop" class="d-flex align-items-center mb-3 mb-md-0 me-md-auto text-dark text-decoration-none">
            <svg class="bi me-2" width="40" height="30">
            ...
            <ul class="nav nav-pills">
               <li class="nav-item">
                  <a href="shop" class="nav-link " >
                     <fmt:message bundle="${loc}" key="language.main"/>
                  </a>
               </li>
               <li class="nav-item">
                  <a href="${pageContext.request.contextPath}/shop?command=show_products&page=1" class="nav-link">
                     <fmt:message bundle="${loc}" key="language.catalog"/>
                  </a>
               </li>
               <c:if test="${sessionScope.role.name == 'unknown'}">
                  <a href="shop?command=go_to_login" class="nav-link">
                     <fmt:message bundle="${loc}" key="language.logIn"/>
                  </a>
               </c:if>
               <li class="nav-item">
                  <a href="shop?command=about" class="nav-link">
                     <fmt:message bundle="${loc}" key="language.about"/>
                  </a>
               </li>
               <c:if test="${sessionScope.role.name != 'unknown'}">
                  <a href="shop?command=logout" class="nav-link">
                     <fmt:message bundle="${loc}" key="language.logout"/>
                  </a>
               </c:if>
               <c:if test="${sessionScope.role.name != 'unknown'}">
                  <a href="shop?command=go_to_profile" class="nav-link">
                     <fmt:message bundle="${loc}" key="language.account"/>
                  </a>
               </c:if>
            <c:if test="${(sessionScope.language == 'ru') && (sessionScope.language != null)}">
            <li><a href="${pageContext.request.contextPath}/shop?${pageContext.request.queryString}&locale=en">EN </a></li>
            </c:if>
             <c:if test="${sessionScope.language == 'en' && (sessionScope.language != null)}">
            <li><a href="${pageContext.request.contextPath}/shop?${pageContext.request.queryString}&locale=ru"> RU</a></li>
            </c:if>
            </ul>
         </h6>
      </header>
